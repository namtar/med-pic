...
public abstract class Abstract_ImagePlugin implements PlugInFilter {
	...
	public void run(ImageProcessor ip) {
		...
		int h = ip.getHeight();
		roi = ip.getRoi();
		inputImageProcessor = ip;

		//        System.out.println("Width: " + w);
		//        System.out.println("Height: " + h);

		// create a new image with the same size and copy the pixels of the original image
		corrected = NewImage.createByteImage("Corrected image", w, h, 1, NewImage.FILL_BLACK);
	...
	public ImageProcessor getOutputImage() {
		return outputImage;
	}

	protected void updateDrawAndShow() {

		corrected.show();
		corrected.updateAndDraw();
	}

	public ImageProcessor getInputImageProcessor() {
		return inputImageProcessor;
	}
}
